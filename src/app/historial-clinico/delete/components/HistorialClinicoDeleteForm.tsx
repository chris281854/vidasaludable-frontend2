...
import useDeleteHistorialClinico from "@/app/hooks/historial-clinico/useDeleteHistorialClinico";
...
import HistorialClinicoModal from "../../edit/components/HistorialClinicoModal";
import ProtectedRoute from "@/components/ProtectedRoute";
import { HiIdentification } from "react-icons/hi2";

const HistorialClinicoDeleteForm: React.FC = () => {
    ...
    const { deleteHistorialClinico, loading, error } = useDeleteHistorialClinico(); // Usar el hook
    ...
    const handleDelete = async () => {
        try {
            ...
            await deleteHistorialClinico(formData.idHistorial, dataToUpdate); // Llamar a la función de guardado
            setSnackbarOpen(true); // Mostrar Snackbar de éxito
            clearForm(); // Limpiar el formulario después de enviar
        } catch (error) {
            console.error('Error al eliminar el historial clínico del paciente:', error);
        ...
        <ProtectedRoute>
            <Box sx={{ padding: 2, mt: 3 }}>
                ...
                <Snackbar
                    ...
                    message={error ? error : 'Historial eliminado con éxito'}
                />
...
export default HistorialClinicoDeleteForm;